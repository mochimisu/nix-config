...
in {
  ...
  programs.zsh.initContent = lib.mkOrder 1500 ''
    ...
    if [[ -n "$SSH_CONNECTION" || -n "$SSH_TTY" || -n "$SSH_CLIENT" || -n "$MOSH_CLIENT" ]]; then
      _kitty_is_kitty=0
      ...
      if [[ -n "$TMUX" ]]; then
        tmux set-option -gq @ssh_bg "$_kitty_bg_hex"
        tmux set-option -gq @ssh_fg "$_kitty_fg"
