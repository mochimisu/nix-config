...
- Gaia's Home Assistant firewall trusts `wpan0` for the local ZBT-2 path and allows `fd0b:6932:1d1a:1::/64` GaiaThread OMR traffic arriving on `enp5s0` so same-dataset Ethernet border routers (Aqara M3, DIRIGERA, and local OTBR advertisements) can deliver Matter/Thread reports back to Gaia.
