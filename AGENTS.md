...
- Linux Home Manager users that import `home/default.nix` start Codex remote control through the `codex-remote-control` systemd user service; Darwin is intentionally excluded. The same module creates `~/.codex/packages/standalone/current/codex` as a symlink to `/run/current-system/sw/bin/codex` because `codex remote-control start` requires that standalone path even when Codex itself is Nix-managed.
