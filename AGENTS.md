...
- Hyprland should come from nixpkgs (`programs.hyprland.enable = true`) rather than a separate `hyprwm/Hyprland` flake input. Keep Hyprgrass active for touchscreen hosts; the touchscreen module translates old hyprgrass bind strings to the PR #381 `hl.plugin.hyprgrass.bind` Lua API.
